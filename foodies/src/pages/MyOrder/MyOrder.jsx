          ...
          <div key={order.id || index} className="col-12">
            <div className="card shadow-sm border-0 rounded-4">
              <div className="card-header bg-light d-flex justify-content-between align-items-center rounded-top-4">
                ...
                <span
                  className={`badge text-uppercase p-2 ${
                    order.orderStatus === 'DELIVERED' ? 'bg-success' :
                    order.orderStatus === 'PREPARING' ? 'bg-warning text-dark' : 'bg-primary'
                  }`}
                >
                  {order.orderStatus}
                </span>
              ...
              <div className="card-body p-4">
                <p className="mb-2"><strong>Total Items:</strong> {order.orderedItems?.length ?? 0}</p>
                <p className="mb-2">
                  <strong>Payment Status:</strong>
                  <span className={`badge ms-2 ${formatPaymentBadge(order.paymentStatus)}`}>
                    {order.paymentStatus || 'N/A'}
                  </span>
                </p>
                <p className="mb-2"><strong>Amount:</strong> <span className="text-success fw-bold">&#8377; {(order.amount ?? 0).toFixed(2)}</span></p>
